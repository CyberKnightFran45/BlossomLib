...
progress?.Invoke(totalRead, maxBytes < 0 ? totalRead : maxBytes);
}
...
<param name="progress"> An optional callback to report progress during processing. </param> */
...
public static void Process(Stream input, Stream output, ReadOnlySpan<byte> arg, BytesTransform2 transform,
                           long maxBytes = -1, 
                           ProgressCallback progress = null)
{
int blockSize = MemoryManager.GetBlockSize(input);
