...
private long ResolveIndex(Index index) => index.IsFromEnd ? (long)_size - index.Value : index.Value;

// Resolve Range for Spans

[MethodImpl(MethodImplOptions.AggressiveInlining)]

private void ResolveRange(Range range, out long start, out int length)
{
start = ResolveIndex(range.Start);
long end = ResolveIndex(range.End);
...
public T ElementAt(Index index) => ElementAt(ResolveIndex(index) );
...
public ReadOnlySpan<T> GetView(Range range)
{
ResolveRange(range, out var start, out var length);

return GetView(start, length);
}
