...
/// <param name="progress">Optional callback for reporting progress.</param>
...
Process(input, output, t, maxBytes, progress);
}

// Encode internal

private static void EncodeInternal(TraceContext ctx, string inputPath,
                                   string outputPath, bool isWebSafe,
...
TraceFileSteps.Run(ctx,
				   ...
				   outputPath,
                   "Encoding data...",
				   (i, o, _) => EncodeStream(i, o, isWebSafe, -1, progress)
);
...
TraceExecutor.Run("Base64 Encode",
                  ctx => EncodeInternal(ctx, inputPath, outputPath, isWebSafe, progress),
                  ("InputPath", inputPath),
...
private static void DecodeInternal(TraceContext ctx, string inputPath,
                                   string outputPath, bool isWebSafe,
                                   ProgressCallback progress)
{
...
TraceFileSteps.Run(ctx,
				   ...
				   outputPath,
                   "Decoding data...",
				   (i, o, _) => DecodeStream(i, o, isWebSafe, -1, progress)
);
...
TraceExecutor.Run("Base64 Decode",
                  ctx => DecodeInternal(ctx, inputPath, outputPath, isWebSafe, progress),
                  ("InputPath", inputPath),
                  ("OutputPath", outputPath),
                  ("IsWebSafe", isWebSafe)
);
